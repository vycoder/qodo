...
import UseStackedBarChart from './mixin-stacked-bar-chart'
...
export default {
  ...
  computed: { ...mapGetters('tasks', ['taskById']) },
  methods: {
    initialize () {
      if (!this.data.length) {
        return
      ...
      const tasks = Array.from(new Set(this.data.map(r => r.task)))
      const series = tasks.map(task => ({
        name: this.taskById(task).name,
        data: dates.map(day =>
          this.data.filter(r => r.datekey === day && r.task === task)
            .map(r => r.progress)
            .reduce((a, b) => a + b, 0)
        )
      }))
